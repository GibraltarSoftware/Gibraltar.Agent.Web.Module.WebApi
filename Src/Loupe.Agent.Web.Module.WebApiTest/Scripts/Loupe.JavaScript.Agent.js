(function (window) {
    ...
    // set up partial method calls for the convenience methods
    ...
    window.loupe = {
        ...
        setAuthorisationHeader: setAuthorisationHeader
    };
    ...
    function setUrlPrefix(prefix) {
        if (prefix) {
            urlPrefix = stripTrailingSlash(prefix);
        }
    }

    function setAuthorisationHeader(header) {
        if (header) {
            if (header.name && header.value) {
                authorisationHeader = header;
            }
        }
    }
    ...
    function createHelpers() {
        ...
        try {
            ...
            consoleLog("Unable to set up stack trace capture functionality: " + e.message);
            stackTrace = null;
        } 
    ...
    function storeMessage(message){
        if(storageAvailable) {
            try{
                ...
                consoleLog("Error occurred trying to add item to localStorageL " + e.message);
        ...
        try {
            ...
            xhr.onreadystatechange = function () {
                if (xhr.readyState === 4) {
                    ...
                    // if the call was successful and we have keys to items in storage
                    // now we remove them
    ...
    function buildUrl() {
        ...
        if (urlPrefix) {
            origin = origin + "/" + urlPrefix;
        }

        return origin + "/loupe/log";
    }
    ...
    function createCORSRequest(url) {
      ...
      if ("withCredentials" in xhr) {
        ...
        if (authorisationHeader) {
            xhr.setRequestHeader(authorisationHeader.name, authorisationHeader.value);
        }
